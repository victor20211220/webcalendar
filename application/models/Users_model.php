...
class Users_model extends Crud_model {
    ...
    function authenticate($email, $password) {
        $this->db->select("id,user_type,client_id,role_id");
        ...
        if ($result->num_rows() == 1) {
			...
			$this->session->set_userdata('team_type',$this->get_team_role($user_info->role_id));
            return true;
        }
    }
	function get_team_role($role_id){
		$role_table = $this->db->dbprefix("roles");
		
		$sql = "SELECT title FROM $role_table WHERE id='$role_id'";
