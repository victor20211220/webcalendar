...
class Projects extends MY_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $start_date = $data->start_date * 1 ? format_to_date($data->start_date, false) : "-";
        $dateline = $data->deadline * 1 ? format_to_date($data->deadline, false) : "-";
    ...
    function task_modal_form() {
        ...
        $view_data['milestones_dropdown'] = array(0 => "-") + $this->Milestones_model->get_dropdown_list(array("title"), "id", array("project_id" => $project_id));
    ...
    function gantt_data($project_id = 0, $group_by = "milestones", $filter_id = 0, $status = "") {
        if ($project_id) {
            ...
            foreach ($gantt_data as $data) {
                ...
                //has deadline? change the color of date based on status
                if ($data->status_id == "1" && $data->end_date * 1 && get_my_local_time("Y-m-d") > $data->end_date) {
                    $color = "#d9534f";
                }
