 ...
 * @param integer $cdate   Timestamp of day in question
 * @param array   $Byxxx   Byxxx array
 *
 */
   ...
   */
    ...
    $ret = ( $can_add ? '
     ...
     . '">' . substr ( $date, 6, 2 ) . '</a>' . ( empty ( $tmp )
      ...
      . '" alt="' . $moon_title . '" />' ) . "<div class='liczba' style='display:none'>".$title2."</div><br /><div class='ordering'>\n";
    $cnt++;
  }
...
function print_entry ( $event, $date ) {
  ...
  $ret .= '
      <a ' . $title . ' class="' . $class . ' ' . $whois . '"   id="' . "$linkid\" $href"
   . '><img src="';
